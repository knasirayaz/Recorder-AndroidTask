...
open class FilePicker(private val applicationId: String) : BaseFilePicker(), OnClickListener {
    ...
    override fun onCreateView(
        ...
        if (!directAction) binding.layout.visibility = VISIBLE
        return binding.root
    }
    ...
    private fun updateUiTheme() {
        if (actionButtonBg != null) {
            with(binding) {
                ...
                btnChooseDocument.setBackgroundResource(actionButtonBg!!)
            }
        }
        if (actionButtonTextColor != null) {
            with(binding) {
                ...
                btnChooseDocument.setTextColor(
                    ContextCompat.getColor(
                        requireContext(),
                        actionButtonTextColor!!
                    )
    ...
    private fun mapping() {
        with(binding) {
            ...
            btnChooseDocument.setText(R.string.choose_document)
        ...
        binding.btnChooseDocument.setOnClickListener(this)
        ...
        binding.btnChooseDocument.visibility = GONE
        ...
        when (fileSelectionType) {
            ...
            IMAGE_DOCUMENT -> {
                binding.btnCaptureImage.visibility = VISIBLE
                binding.btnChooseImage.visibility = VISIBLE
                binding.btnChooseDocument.visibility = VISIBLE
            }
    ...
    override fun onClick(view: View) {
        when (view) {
            ...
            binding.btnChooseDocument -> {
                directActionType = IMAGE_DOCUMENT
                selectFile()
            }
        }
    }
...
//            selectFile()
                ...
                try {
                    if (result.data?.data != null) {
                        val file = if (documentWithOriginalName)
                            getFileFromUriWithOriginal(mContext, result.data?.data, FileType.AUDIO)
                        else getFileFromUri(mContext, result.data?.data, DOCUMENT)
                        if (file != null)
                            media = Media.create(generateThumb(FileType.AUDIO, file))
                    }
                } catch (e: Exception) {
                    e.printStackTrace()
                    media = null
    ...
    private fun sendCallBack(media: Media?) {
        ...
        executeOnMain {
            if (media == null) callback?.onError("Media not Found!")
            else callback?.onSuccess(media)
        }
        hideBottomSheet()
    }

    private fun sendErrorCallBack(msg: String) {
        showProgressBar(false)
        executeOnMain { callback?.onError(msg) }
        hideBottomSheet()
    }
    ...
    private fun sendCallBack(mediaList: ArrayList<Media>?) {
        showProgressBar(false)
        executeOnMain {
            if (mediaList.isNullOrEmpty()) callback?.onError("Media not Found!")
    ...
    private fun cropImage(file: File?) {
        if (file == null) {
            sendErrorCallBack("Unable to crop image")
            return
        }
        var cropFile = File(
    ...
    * customize methods
    * */
    fun type(fileSelectionType: FileSelectionType): FilePicker {
        this.fileSelectionType = fileSelectionType
        return this
    }

    fun enableDirectAction(directActionType: FileSelectionType): FilePicker {
     ...
     */
    fun enableCrop(): FilePicker {
        this.crop = true
        return this
    }

    fun crop(x: Float, y: Float): FilePicker {
        cropX = x
        cropY = y
        return enableCrop()
    }

    fun cropSquare(): FilePicker {
        return crop(1f, 1f)
    }

    fun maxCropResultSize(width: Int, height: Int): FilePicker {
        this.maxCropWidth = width
        this.maxCropHeight = height
        return this
    }

    fun callBack(callBack: FilePickerCallback): FilePicker {
        this.callback = callBack
        return this
    }

    fun compressQuality(quality: Int): FilePicker {
        this.quality = quality
        return this
    }

    fun compressedFormat(format: CompressFormat): FilePicker {
        this.compressFormat = format
        this.isCompress = true
        return this
    }

    fun setResolutionConstraint(width: Int, height: Int): FilePicker {
        this.isCompress = true
        compressResolution = ResolutionConstraint(width, height)
        return this
    }

    fun pickDocumentType(mimeTypes: Array<String>): FilePicker {
        this.mimeTypes = mimeTypes
        return this
    }

    fun enableMultiSelection(): FilePicker {
        this.isMultiSelection = true
        return this
    }

    fun enableDocumentWithOriginalName(): FilePicker {
    ...
    companion object {
        ...
        fun with(context: Context): FilePicker {
            return FilePicker(context.packageName)
        }

        fun with(context: Context, @LayoutRes layoutRes: Int): FilePicker {
            val filePicker = with(context)
            filePicker.customLayoutRes = layoutRes
            return filePicker
        }

        fun with(context: Context, fileSelectionType: FileSelectionType): FilePicker {
            val filePicker = with(context)
